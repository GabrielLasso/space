...
/* Em todas as funes de desenho e cia de regi varieis dos tipos
...
#ifndef NOXPM
/* XPM */
/* No formato Xpm, a cor transparente indicada por
 * 'None'.  Esta informao usada para especificar
 * uma mcara ao mesmo tempo que se indica a figura
 */

/* Lum arquivo xpm e retorna o PIC correspondente 
 * retorna a mascara associada em 'm', se este for
 * diferente de zero.
 */
PIC ReadPic(WINDOW *w, char *fname, MASK m);

/* Idem, mas lda memia ao inv do arquivo */
PIC MountPic(WINDOW *w, char **data, MASK m);

/* escreve o conte�do do PIC p no arquivo fname
...
/* Verifica se existe alguma tecla disponel */
int WCheckKBD(WINDOW *w);

/* pega a prima tecla e retorna o cigo X.
 * Veja em $INC/X11/keysymdef.h */
KeyCode WGetKey(WINDOW *w);

/* Retorna o KeySym da �ltima tecla lida */
KeySym WLastKeySym();
...
/*Atualiza o evento*/
XEvent WGetEvent(WINDOW *w);

/*Devolve o ultimo evento lido*/
XEvent WLastEvent();

/*Converte keycode em keysym*/
KeySym KeyConvert(KeyCode code);
